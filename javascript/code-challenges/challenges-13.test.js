...
const longestString = (arr) => {
  ...
  if (arr.length) {
    ...
    const maxWordLength = arr.map(word => word.length).reduce((acc, currentValue) => acc > currentValue ? acc : currentValue);
    return wordLength.findIndex(element => element === maxWordLength);
  } else {
    return -1;
  }
...
const standardizePhoneNumbers = (arr) => {
  // Solution code here...
  const regex = /[^\d]/gm;
  return arr.map(number => number.replaceAll(regex, ''));
};
...
const onlyOddChars = (str) => {
  ...
  for (let i = 0; i < str.length; i++) {
    if (i % 2 !== 0) {
      oddString.push(str[i]);
    }
  }
  // console.log(oddString);
  return oddString.join().replaceAll(regex, '');
};
...
const allHappy = (arr) => {
  // Solution code here...
  return arr.map(word => word.includes(':)')).every(element => element === true);
};
