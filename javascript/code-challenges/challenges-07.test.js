...
const listFoods = (recipe) => {
  ...
  //   result.push(recipe.ingredients[i].slice(2));
  //  }
  let keywords = ['Gruffalo', 'oats', 'brown sugar', 'flour', 'pure maple syrup', 'chopped nuts', 'baking soda', 'baking powder', 'cinnamon', 'melted butter', 'fresh water'];
  let sliceIdx = [];
  for(let i = 0; i < recipe.ingredients.length; i++){
    let idx = recipe.ingredients[i].indexOf(keywords[i]);
    sliceIdx.push(idx);
  }
  for(let i = 0; i <recipe.ingredients.length; i++){
    result.push(recipe.ingredients[i].slice(sliceIdx[i]));
  }
  return result;
};
