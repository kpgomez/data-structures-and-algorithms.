...
function transformToLis(obj) {
  // Solution code here...
  let transformedObj = Object.entries(obj);
  let newArray = [];
  for (let i = 0; i < transformedObj.length; i++) {
    newArray.push(`<li>${transformedObj[i][0]}: ${transformedObj[i][1]}</li>`);
  }
  return newArray;
}
...
const count = (target, input) => {
  ...
  for (let i = 0; i < input.length; i++) {
    input[i].filter(element => element === target).forEach(item => emptyArray.push(item));
  }
...
const totalSum = (input) => {
  // Solution code here...
  return input.map(array => array.reduce((acc, currentValue) => acc += currentValue, 0)).reduce((acc, currentValue) => acc += currentValue);
};
...
//https://www.mathsisfun.com/divisibility-rules.html#:~:text=Note%3A%20Zero%20is%20divisible%20by,yes%22%20to%20all%20these%20tests.
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  return input.map(array => array.filter(element => element % 5 === 0 && typeof(element) === 'number')).map(array => array.map(element => Math.pow(2, element)));
}
...
let findShortest = (data) => {
  // Solution code here...
  let shorty = data.map(obj => obj.height).map(element => parseInt(element)).reduce((acc, currentValue) => acc < currentValue ? acc : acc = currentValue);
  return data.filter(obj => obj.height === shorty.toString()).map(obj => obj.name).toString();
};
